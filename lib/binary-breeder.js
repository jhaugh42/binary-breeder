...
module.exports = function(parentChromosomes, numOffspring, mutationChance) {

    if(numOffspring < 2) {
        throw new Error('The breeder produces a minimum of two offspring.')
    }
    ...
    for(var i = 1; i < numOffspring; ++i) {
        ...
        offspring.forEach(function(child) {
            var mutated = mutate(child, mutationChance);
            newChildren.push(mutated);
        });
    }

    return killRandomChildren(newChildren, numOffspring);
